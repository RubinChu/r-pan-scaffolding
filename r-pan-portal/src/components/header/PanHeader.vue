...
<script>
    ...
    export default {
        ...
        methods: {
            initUserInfo() {
                let _this = this
                userService.info(function (res) {
                    if (res.status == 0) {
                        _this.username = res.data.username
                        _this.$emit('callback', res.data.rootFileId, res.data.rootFileName)
                    } else {
        ...
        watch: {
            changePasswordDialogVisible: function (newValue, oldValue) {
                if (newValue) {
                    this.$nextTick(() => {
