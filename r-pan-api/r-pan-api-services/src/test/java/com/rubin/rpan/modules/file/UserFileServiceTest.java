...
public class UserFileServiceTest {
    ...
    public void uploadSuccessTest() {
        ...
        List<RPanUserFileVO> rPanUserFileVOList = iUserFileService.upload(multipartFile, rPanUserVO.getRootFileId(), userId, "111", multipartFile.getSize());
        Assert.assertEquals(1, rPanUserFileVOList.size());
    }
    ...
    public void uploadWithChunkSuccessTest() {
        ...
        for (int i = 0; i < chunks; i++) {
            new Thread(() -> {
                ...
                MultipartFile multipartFile = generateMultipartFile();
                List<RPanUserFileVO> rPanUserFileVOList = iUserFileService.uploadWithChunk(multipartFile,
                        rPanUserVO.getRootFileId(),
                        userId,
                        "111",
                        chunks,
                        chunk,
                        multipartFile.getSize() * chunks,
                        "test.txt");
                if (CollectionUtils.isNotEmpty(rPanUserFileVOList)) {
                    System.out.println(chunk);
                    System.out.println(rPanUserFileVOList);
                }
    ...
    public void transferTargetFileIsNotFolderFailTest() {
        ...
        Assert.assertEquals(1, rPanUserFileVOList.size());
        MultipartFile multipartFile = generateMultipartFile();
        rPanUserFileVOList = iUserFileService.upload(multipartFile, rPanUserVO.getRootFileId(), userId, "111", multipartFile.getSize());
        Assert.assertEquals(2, rPanUserFileVOList.size());
