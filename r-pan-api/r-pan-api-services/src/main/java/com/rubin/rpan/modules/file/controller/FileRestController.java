...
public class FileRestController {
    ...
    @NeedLogin
    public R<List<RPanUserFileVO>> upload(@Validated FileUploadPO fileUploadPO) {
        if (fileUploadPO.isChunked()) {
            return R.data(iUserFileService.uploadWithChunk(fileUploadPO.getFile(), fileUploadPO.getParentId(), UserIdUtil.get(), fileUploadPO.getMd5(), fileUploadPO.getChunks(), fileUploadPO.getChunk(), fileUploadPO.getSize(), fileUploadPO.getName()));
        }
        return R.data(iUserFileService.upload(fileUploadPO.getFile(), fileUploadPO.getParentId(), UserIdUtil.get(), fileUploadPO.getMd5(), fileUploadPO.getSize()));
    }
