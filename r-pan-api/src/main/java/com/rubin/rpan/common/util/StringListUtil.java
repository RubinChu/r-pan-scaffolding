...
public class StringListUtil {
    ...
    public static List<Integer> string2IntegerList(String origin) {
        return Splitter.on(CommonConstant.COMMON_SEPARATOR).splitToList(origin).stream().map(Integer::valueOf).collect(Collectors.toList());
    }
    ...
    public static List<Long> string2LongList(String origin) {
        return Splitter.on(CommonConstant.COMMON_SEPARATOR).splitToList(origin).stream().map(Long::valueOf).collect(Collectors.toList());
    }
    ...
    public static String longListToString(Long... ids) {
        return StringUtils.join(Arrays.asList(ids), CommonConstant.COMMON_SEPARATOR);
    }
